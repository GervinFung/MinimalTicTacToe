...
const horizontalCheck = (
    ...
    const score = chuckedTileList
        .map((tileList) => {
            return tileList
                .map((tile) =>
                    computeScoreFromLeague(tile.getPiece?.league, league)
                )
                .reduce(reducer);
        })
        .reduce(reducer);
        ...
        .map((tileList, index) => {
            return tileList
                .map((_, tileIndex) => {
                    ...
                    return computeScoreFromLeague(
                        anotherTile.getPiece?.league,
                        league
                    );
                })
                .reduce(reducer);
