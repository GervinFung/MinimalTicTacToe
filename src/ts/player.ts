...
const createLegalMoves = (
    league: League,
    tileList: ReadonlyArray<Tile>
): ReadonlyArray<Move> =>
    tileList
        .filter((tile) => !tile.isTileOccupied)
        .map((tile) => createMove(league, tile.index));
...
export const createFirstPlayer = (tileList: ReadonlyArray<Tile>) =>
    createPlayer(League.FIRST, tileList);
export const createSecondPlayer = (tileList: ReadonlyArray<Tile>) =>
    createPlayer(League.SECOND, tileList);

export const makeMoveFromMoves = (movePassed: Move, board: Board) => {
    const moveFound = board.currentPlayer.legalMoves.find((move) =>
        equals(move, movePassed)
    );
    return makeMove(board, getMove(moveFound).piece);
};

export const makeMoveFromTileNumber = (
    tileNumber: number,
    board: Board
): Board => makeMove(board, getMoveFromTileNumber(tileNumber, board).piece);

export const getMoveFromTileNumber = (
    tileNumber: number,
    board: Board
): Move => {
    const moveFound = board.currentPlayer.legalMoves.find(
        (move) => move.piece.index === tileNumber
    );
